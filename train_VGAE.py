...
def main(args):
	...
	encoder_args = {"input_dim": 7, "output_dim": args.hidden_dim}
	encoder = globals()[args.encoder_name](**encoder_args).to(args.device)
	decoder = EdgeDecoder(input_dim=args.hidden_dim, output_dim=1)
	...
	for epoch in range(1, args.num_epoch+1):
		...
		for batch in train_loader:
			...
			loss = model.recon_loss(z, batch.edge_index, batch.edge_type)
			loss += model.kl_loss()
			...
			acc = model.test(z, batch.edge_index, batch.edge_type)
			acc_train += acc
