...
def main(args):
	...
	# dataset, split
	d = RoomAsNodeClassificationDataset(root=args.data_path, data_num=args.data_num)
	...
	model_args = {"input_dim": 8+GRAPH_BASED_FEATURE_DIM, "hidden_dim": args.hidden_dim, "output_dim": 4,
			 	  "num_layers": args.num_layers, "message_passing": args.message_passing, "sigmoid": True}
	...
	for epoch in range(1, args.num_epoch+1):
		...
		for i, batch in enumerate(train_loader):
			...
			acc_train += binary_classification_accracy(pred, batch.y)
		...
		with torch.no_grad():
			...
			for batch in valid_loader:
				...
				iter_eval += 1
				loss_eval += loss.item()
				acc_eval += binary_classification_accracy(pred, batch.y)
	...
	# plot curves
	plot_learningCurve_classification(acc_record, args.ckpt_dir)
	...
	for i in range(args.save_num):
		...
		output_graph_classification(save_path, graph, pred)
...
def parse_args() -> Namespace:
	...
	parser.add_argument(
		...
		default="./Results/RoomAsNodeClassification",
	)
