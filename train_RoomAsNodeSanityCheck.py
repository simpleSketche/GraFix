...
def main(args):
	...
	# dataset, split
	d = RoomAsNodeSanityCheckDataset(root=args.data_path, data_num=args.data_num)
	...
	for epoch in range(1, args.num_epoch+1):
		...
		with torch.no_grad():
			...
			for i, batch in enumerate(valid_loader):
				...
				if i == 0:
					logger.info("validation prediction:")
					logger.info(pred)
					logger.info("validation y:")
					logger.info(batch.y)
					logger.info("")
...
def parse_args() -> Namespace:
	...
	parser.add_argument(
		...
		default="./Results/RoomSanityCheck/",
	)
