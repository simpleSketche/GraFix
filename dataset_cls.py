...
class VerticesAsNodeDataset(Dataset):
    ...
    def _construct_graph(self, data_index: str):
        ...
        y_lbl = torch.zeros((node_number, 1)).to(torch.float)
        tol=1e-3
        for node_index in range(node_number):
            y[node_index, 0] = nodes_out_data[node_index][0] - x[node_index, 0]
            y[node_index, 1] = nodes_out_data[node_index][1] - x[node_index, 1]
            delta = max([abs(y[node_index, 0])-tol,abs(y[node_index, 1]-tol)])
            if delta>0:
                y_lbl[node_index,0]=1       
        ...
        # graph
        graph = Data(x=x, edge_index=edge_index, y=y_lbl, vertices_movement_prediction=None, original_x=None, original_y=None)
        return graph
